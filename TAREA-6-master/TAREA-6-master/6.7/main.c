#include <stdio.h>
 ...
 void Lecturta1 (int [15][12]);
 void Lecturta2(float *, int);
...
void Lectura1(int A[][12])
 /* Esta función se utiliza para almacenar en el arreglo bidimensional las diferentes transacciones que representan las ventas de los diferentes productos.
 El fin de datos está dado por –1. */
 {
     int MES, PRO, CAN;
     printf("\nIngrese me, tipo de producto y cantidad vendida: ");
 ...
 void Lectura2(float A[], int N)
  /* Esta función se utiliza para leer los precios de venta de los diferentes productos. */
  {
      ...
      for (I=0; I<N; I++)
      {
          printf("Ingrese costodel producto %d: ", I+1);
          scanf("%f", &A[I]);
      }
  }
     ...
     /* Esta función se utiliza para obtener el tipo de producto que más se ha vendido y el monto de las ventas de dicho producto. */
 {
     ...
     printf("\n\nTipo de Producto mas vendido: %D \T Ventas: %.2f", TPR + 1, VEN);
 }
